    ...
    <nav role="navigation">
        ...
        <div class="container">
            ...
            <div class="user_list_table">
                ...
                $sql = "SELECT id, username, name FROM user";
                $exc = $con->query($sql);
                $nbrow = 0;
                echo "<table border=1 align='center' class='table'><thead class='.thead-dark'><th>ID</th><th>Username</th><th>First name</th><th>Delete</th></thead>";
                while ($row = $exc->fetch()) {
                    echo "<tbody><tr><td>" . $row["id"] . "</td><td>" . $row["username"] . "</td><td>" .  $row["name"] . "</td><td>" . "<a class='delite' href='delete_admin.php?id=" . $row["id"] . "'> Delete </a>" . "</td></tr>";

                    $nbrow++;
                }
                echo "<tr><td colspan=4>Ukupno ima: " . $nbrow . " Users </td></tr></tbody></table>";
                ?>
            </div>
        </div>
    </main>
